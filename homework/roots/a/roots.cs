...
public static class roots{
	...
	public static vector NewtonRaphson(Func<vector,vector> f, vector x0, int max=2000, double eps=1e-4){
		vector x = x0.copy();
		for (int i=0; i<x0.size; i++){
			if(x[i] == 0){
				x[i] = 1e-5; //prevents division by 0 but keeps value around abs acc
			}
		}
...
//		if(!converged){
//			Console.Error.Writeline("No convergence reached - using last calculated result.");
//		}
